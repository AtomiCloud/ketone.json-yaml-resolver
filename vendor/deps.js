...
function baseMerger(context, ...sources) {
  ...
  if (Array.isArray(target) && Array.isArray(source)) {
    target.push(...source);
    if (context.options.arrayPriority === "right")
    ...
    for (let i = 0;i < keys.length; i++) {
      ...
      if (hasOwnProperty(target, key)) {
        if (context.options.strategy) {
          if (typeof context.options.strategy(target, key, source[key]) !== "undefined")
            continue;
        }
        if (isObject(target[key]) && isObject(source[key])) {
          ...
          if (context.options.priority === "right")
        ...
        if (context.options.array && Array.isArray(target[key]) && Array.isArray(source[key]))
          switch (context.options.priority !== context.options.arrayPriority ? togglePriority(context.options.arrayPriority) : context.options.arrayPriority) {
            case "left":
